...
(
    ...
    (
        ...
        .cfg_bits_i         ( regs_q[LCR][1:0]              ),
				.cfg_parity_sel_i   ( regs_q[LCR][4]                ),
        .busy_o             (                               ),
        .err_o              ( parity_error                  ),
        .err_clr_i          ( regs_q[MCR][4]                ), //MCR[4] - controls error clear bit 
        .rx_data_o          ( rx_data                       ),
        .rx_valid_o         ( rx_valid                      ),
        .rx_ready_i         ( rx_ready                      )
    );
    ...
    (
        ...
        .clr_rda_i          ( clr_int[3]                    ),
        .clr_line_i         ( clr_int[2]                    ),
        .clr_thre_i         ( clr_int[1]                    ),
        .cfg_div_i          ( {regs_q[DLM + 'd8], regs_q[DLL + 'd8]} ),
        .interrupt_o        ( event_o                       ),
        .IIR_o              ( IIR_o                         )
    );
    ...
    //soft reset logic
		always_ff @(posedge CLK or negedge RSTN) begin
		  if(~RSTN) begin
			  ...
			  tx_rst <= 1'b1;
		    rx_rst <= 1'b1;
			  if(regs_q[LCR][5]) begin
				  tx_rst <= 1'b0;
			  end
				if(regs_q[LCR][6]) begin
				  rx_rst <= 1'b0;
			  end
			end
		end
   // Since original code had logic [3:0] register_adr;
    logic [7:0] active_pwdata;
    logic [7:0] active_prdata;

    // Decode PSTRB and PADDR to get the exact 0-7 register index
    always_comb begin
        register_adr  = 4'h0;
        active_pwdata = 8'h00;
        if (PSTRB[0]) begin
    ...
    begin
        ...
        // rx status
        regs_n[LSR][0] = fifo_rx_valid; // fifo is not empty

        // parity error on receiving part has occured
        regs_n[LSR][2] = parity_error; // parity error is detected when element is retrieved
        ...
        begin
            case (register_adr)
                ...
                begin
                    ...
                    begin
                        regs_n[DLL + 'd8] = active_pwdata;
                    end
                    else
                    begin
                        fifo_tx_data = active_pwdata;
                        fifo_tx_valid = 1'b1;
                    end
                end
                ...
                begin
                    if (regs_q[LCR][7]) // Divisor Latch Access Bit (DLAB)
                        regs_n[DLM + 'd8] = active_pwdata;
                    else
                        regs_n[IER] = active_pwdata;
                end

                LCR:
                    regs_n[LCR] = active_pwdata;
                MCR:
                    regs_n[MCR] = active_pwdata;
                FCR: // write only register, fifo control register
                begin
                    rx_fifo_clr_n   = active_pwdata[1];
                    tx_fifo_clr_n   = active_pwdata[2];
                    trigger_level_n = active_pwdata[7:6];
                end
   ...
   // register read logic
    ...
    begin
        ...
        begin
            case (register_adr)
                ...
                begin
                    if (regs_q[LCR][7]) // Divisor Latch Access Bit (DLAB)
                        active_prdata = regs_q[DLL + 'd8];
                    else
                    begin
                        fifo_rx_ready = 1'b1;
                        active_prdata = fifo_rx_data[7:0];
                ...
                begin
                    active_prdata = regs_q[LSR];
                    clr_int = 4'b1100; // clear parrity interrupt error
                end

                LCR: // Line Control Register
                    active_prdata = regs_q[LCR];

                IER: // either IER or DLM
                begin
                    if (regs_q[LCR][7]) // Divisor Latch Access Bit (DLAB)
                        active_prdata = regs_q[DLM + 'd8];
                    else
                        active_prdata = regs_q[IER];
                end

                IIR: // interrupt identification register read only
                begin
                    active_prdata = {1'b1, 1'b1, 2'b0, IIR_o};
            ...
            // Route the 8-bit read data to the correct PRDATA byte lane based on the strobe
            if      (PSTRB[0]) PRDATA[7:0]   = active_prdata;
            else if (PSTRB[1]) PRDATA[15:8]  = active_prdata;
            else if (PSTRB[2]) PRDATA[23:16] = active_prdata;
            else if (PSTRB[3]) PRDATA[31:24] = active_prdata;
        end
    end
    ...
    // not supporting transfer failure
    assign PREADY  = 1'b1;
    assign PSLVERR = 1'b0;
